# TAKEN FROM: https://github.com/andreasveit/coco-text/blob/master/coco_evaluation.py
...
# Compute detections
def getDetections(groundtruth, evaluation, imgIds = None, annIds = [], detection_threshold = 0.5, score_threshold = None, area_fraction_threshold = None):
    """
    ...
    for cocoid in imgIds:
        ...
        img_meta = groundtruth.loadImgs([cocoid])[0]
        img_area = img_meta['height'] * img_meta['width']
        ### Score threshold for predicts ###
        if score_threshold is not None:
            eval_bboxes = [
                ev_box_id for ev_box_id in eval_bboxes
                if evaluation.anns[ev_box_id]['score'] >= score_threshold
            ]
        ####################################

        ### Area threshold for gt ####
        if area_fraction_threshold is not None:
            gt_bboxes = [
                gt_box_id for gt_box_id in gt_bboxes
                if (area(groundtruth.anns[gt_box_id]['bbox']) / img_area) >= area_fraction_threshold
            ]
        ##############################
